...
import { _greek, _eq, _op, ops_, is_tree, is_numeric, is_string, is_mixed } from './is.js'
import { precedence, func, trig, mathFunc, color } from './expression.js'
...
function brackets(parent, side) {
	...
	if (is_tree(t)) {
		...
		else if ('fghBN'.indexOf(parent.op) !== -1 || color(parent.op) || parent.op === '∫' || parent.op === 'Φ' || parent.op === '|' || parent.op === '/' || parent.op === '√' || (side === 'right' && parent.op === '^')) ret = false
...
function output_tree(tree, side, rt, f) {
	...
	if (!is_tree(t)) {
		...
		if (typeof op !== 'string') debug('error')({ mj: { left, op, right, tree }, k: fm.error_k })
			...
			if (left) {
				let lims = left.split(',')
				if (lims.length === 3) ret = "\\displaystyle∫_{" + lims[0] + '}^{' + lims[1] + '}' + right + lims[2].replace('d', '\\;\\text{d}')
				else if (lims.length === 2) ret = "\\displaystyle∫_{" + lims[0] + '}^{' + lims[1] + '}' + right
				else if (lims.length === 1) ret = "\\displaystyle∫" + right + lims[0].replace('d', '\\;\\text{d}')
				else ret = "∫" + right
			}
			else {
				if (right.indexOf('\\dfrac') !== -1) ret = "\\displaystyle∫" + right
				else ret = "∫" + right
			}
		}
		else if (op === '[]') {
