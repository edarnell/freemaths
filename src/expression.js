...
import { _greek, is_tree, _eq, tree, is, is_recur, is_numeric, is_angle, is_negative, is_mixed, _op } from './is'
import { evaluate, var_error } from './evaluate'
import Tokens from './Tokens'
import { pruneTree, change_sign, compare_tree } from './compare'
	...
	if ((rhs || rhs === 0) && !var_error(rhs)) {
		...
		if (is_tree(lhs)) {
			if (_f_[lhs.op]) {
				...
				const v = lhs.op
				// TODO don't evaluate - will allow points to be overridden before and after, could also name.
				// e.g. n={0..10} x_0=1 x_{n+1}=2x_{n} x_n=/
				if (algebra(v, vars) && v.charAt(v.length - 1) === '_') {
					const n = lhs.left, ns = n && evaluate(n, vars), f = lhs.right
					if (Array.isArray(ns) && f) {
						const _n = vars._v_[n] // copy before loop
						vars._v_[v + n] = []
						for (var i = 0; i < ns[0] && i < 5; i++) if (vars._v_[v + i]) vars._v_[v + n].push(vars._v_[v + i])
						ns.forEach(x => {
							vars._v_[n] = x
							const vx = v + evaluate(f, vars)
							vars._v_[vx] = evaluate(vars._x_[vx] ? vx : rhs, vars) // _x_ will allow individual values to be replaced
							vars._v_[v + n].push(vars._v_[vx])
						})
						vars._v_[n] = _n // replace after loop
					}
					debug('a_{n}')({ v, n, ns, f, vars })
				}
			}
		}
		...
		//else if (is_tree(lhs) && lhs.op===',' && algebra(lhs.right,vars)) vars[lhs.right]=rhs // or could alter =, precedence
			...
			if (is_tree(rhs) && rhs.op === '{}') vars._v_[x] = evaluate(rhs.right, vars)
			else vars._v_[x] = rhs
		}
	}
...
function expression_1(lhs, min_precedence, tokens) {
	...
	while (lookahead && parseError === '' && lookahead !== ')' && lookahead !== '}' && !(mod === '|' && lookahead === '|') && lookahead !== ']' && precedenceX(lookahead, tokens) >= min_precedence) {
		...
		if (op === "'") {
			...
			if (!condition && (op === '=' || op === '~' || op === '≈')) {
				debug('=')({ op, rhs, tokens })
				const ev = rhs === '#' || rhs === '/' || rhs === '˽' // TODO code ^ and extend options
				if (ev) {
					let n, dpsf, right = 'sf'
					if (rhs === '#') {
						n = '' // special case 10 sig figs but truncate zeros
						if (is_numeric(lookahead)) {
							n = tokens.next()
							lookahead = tokens.peek()
							if (lookahead === 'd' || lookahead === 'b' || lookahead === 's' || lookahead === 't') {
								const r = { d: 'dp', b: 'base', s: 'sf', t: 'tp' }
								right = r[lookahead]
								tokens.next()
								lookahead = tokens.peek()
							}
						...
						const t = (right === 'base') ? tree('', 'base', n) : tree(n, ' ', right)
						rhs = evaluate(tree(prev ? prev : lhs, '#', t), tokens.vars)
					}
					else if (rhs === '˽') {
						rhs = evaluate(tree(prev ? prev : lhs, '#', 'mixed'), tokens.vars)
					}
					else rhs = evaluate(prev ? prev : lhs, tokens.vars)
					debug('eval')(rhs, n, dpsf, lookahead)
				}
	...
	if (is_tree(lhs)) {
		if (lhs.op === ',' && lookahead === ')') lhs.op = '(,)' // coords
...
function primary(tokens) {
	...
	if (ret === '(' || ret === '(.') {
		...
		const l = expression_1(primary(tokens), 0, tokens)
		if (tokens.peek() !== ']') error(']', tokens) // error if not ')';
		else {
			tokens.next();
			ret = tree('', "lim", l);
		...
		condition = true
		const set = tokens.peek() === "," ? tokens.next() : undefined
		ret = expression_1(primary(tokens), 0, tokens)
		if (tokens.peek() !== '}') error('}', tokens) // error if not ')';
		else {
			tokens.next()
			if (set || (!tokens.vars.set && is_tree(ret) && (ret.op === ',' || ret.op === '..'))) ret = tree('', '{}', ret)
			//else if (is_tree(ret) && ret.op === '..') ret = tree('', '{}', ret) // for e.g. n={1..100} display as is but n will be set to {1,2,3,...}
			else {
				let val = evaluate(ret, tokens.vars)
				debug('{}', tokens.vars.set)({ ret, val })
				...
				if (is_numeric(val)) {
					if (val * 1 === 0) ret = '0.0' // magic values for pruning
						...
						if (val.toString().length > 15) {
							// deal with 0.999999x and 0.000000000x could also deal with 0.6666666x where x!=7
							const d = parseFloat(val).toPrecision(15) * 1
							ret = d.toString().length < 15 ? d : val
						}
						else ret = val
					}
	...
	else if (algebra(ret) && ret.endsWith('_') && tokens.peek() === '{') {// e.g. a_{n+1}
		tokens.next()
		const n = primary(tokens), // assumes first term is iteration variable
			sub = expression_1(n, 0, tokens)
		if (!sub || tokens.peek() !== '}') error(ret, tokens)
		else {
			tokens.next()
			ret = tree(n, ret, sub)
		}
	...
	else if (ret === '#' || ret === '˽' || ret === '/') { } // ret=ret for evaluate && (!tokens.peek() || tokens.peek() === '≈')
...
function mathFunc(op, calc) {
	...
	return (calc || condition) && ['random', 'rand', 'round', 'min', 'max', 'trunc', 'floor', 'ceil', 'abs', 'var', 'pcc', 'line'].indexOf(op) !== -1
}
...
function str_token(ts) {
	...
	for (var t = 0; t < ts.tokens.length; t++) {
		...
		if (x === '{') cond = true // special case for a_{n+1}
		else if (x === '}') cond = false
		if (_x_[x]) {/*_x_[x] = _x_[x]*/ }
		else if (x.charAt(0) === '"') {
			...
			else if (ts.vars.auto && /^[a-zABD-Z]$/.test(x) && !_x_[x] && !_f_(ts, x, t) && !(cond && (keyword(x) || func(x, null, true)))) _x_[x] = x // not C for nCr
		}
...
function precedence(op, vars) {
	if (op === '#') return 0.5
	else if (op === "=" || op === '<' || op === '>' || is(_eq, op) || op === '∫' || op === 'Σ' || (!condition && op === '~')) return 1
	...
	else if (op === '*' || op === ' ' || op === '(' || op === '|' || op === '×' || op === '⋅' || op === '..') return 4 // 3
