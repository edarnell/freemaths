...
function dateTime(ts, year, sec) {
  ...
  return dt ? (year ? ddmmyy(dt) : ddmm(dt)) + ' ' + (sec ? hhmmss(dt) : hhmm(dt)) : ' '
}
...
export { _fm, set, update, remote, fontSize, istrue, empty, isset, ddmmyy, hms, date, ddmm, hhmm, date_wmy, time, ts, dateTime, dateInput, copy, ocopy, LC1, UC1, values, debug, set_debug, debugs, zip, unzip, fm, equal, getWeek, hm, sleep }
