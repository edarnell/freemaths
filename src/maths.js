import { expression, set_vars } from './expression'
import { var_error } from './evaluate'
import { mj, replace_symbols } from './mj'
import { copy, debug, fm } from './Utils'
import { is_numeric } from './is'
...
var line1 = null
function get_line1() { return line1 }
function set_line1(l) { line1 = l }
function maths(text, vars, auto) {
	...
	let chunks = [], hide
	let p = { token: null, pos: 0, vars, auto: auto, stop: ['Be'], text }, tree
	let clear = false
	const units = set_units()
	if (line1 && text.startsWith(line1.text)) {
		p.vars = JSON.parse(line1.vars)
		p.pos = line1.text.length
	} else line1 = null
	while (next_tok(p, clear) !== null) {
		if (p.token === '\n' && !line1 && text.startsWith('//')) {
			line1 = { vars: JSON.stringify(p.vars), text: p.text.substring(0, p.pos) }
		}
	...
	fm.maths_timer = 0
	//debug('vars', true)({ saved: copy(_vars) })
	return chunks
}
...
export { maths, replace_vars, get_line1, set_line1 } // line1 used to sync with remote
