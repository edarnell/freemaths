...
import { ops, ops_, is, is_angle, is_recur, is_mixed } from './is'
import { func, set_vars, algebra } from './expression'
	...
	constructor(maths, vars, force = false) {
		...
		if (maths) for (var i = 0; i < maths.length; i++) {
			...
			if (maths[i] === '"') {
				...
				if (token.length > 0 && (is_recur(token) !== is_recur(maths[i]) || token === '..')) // .. hack for now
